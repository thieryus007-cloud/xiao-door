 ...
 * Le bouton physique de la carte (button0/sw0, câblé au niveau matériel,
 * cf. devicetree) n'est jamais lu par ce firmware -- envoyé à 0.
 * Le yaw nécessiterait une intégration gyroscopique dans la durée avec
 * recalage à l'inactivité (§7.7) : non calculé, envoyé à 0 (le
 * gyroscope, lui, est bien lu -- voir trame C).
...
/* Fenêtre active bornée par prudence (cf. en-tête) : ~30 min de sondage 2s
 * continu au maximum avant de forcer un retour en System OFF, même si le
 * capteur reste "occupé" en continu (vibration soutenue, capteur en
 * défaut...). Ne devrait jamais être atteint en usage normal. */
#define ACTIVE_WINDOW_MAX_ITERS  900
			   ...
			   const char *reason)
{
	...
	frame_a[A_OFF_BUTTON] = 0;    /* bouton physique câblé mais non lu */
...
static void send_frame_c(const struct device *imu, const struct imu_sample *accel)
{
	...
	gyro_ret = read_gyro_burst(imu, &gx, &gy, &gz);
	if (gyro_ret == 0) {
				...
				bool triggered_by_motion)
{
	...
	while (1) {
		...
		if (++iters > ACTIVE_WINDOW_MAX_ITERS) {
			LOG_WRN("fenêtre active bornée atteinte (%d cycles), retour forcé en veille",
				ACTIVE_WINDOW_MAX_ITERS);
			break;
		}
...
int main(void)
{
	...
	if (gpio_wake || heartbeat_due) {
		run_active_window(imu, heartbeat_due, gpio_wake);
	...
	diag_log_append(reset_cause, fresh_session, gpio_wake, wake_in_us,
			 ret_gpio_wake, ret_grtc_prepare);
	...
	/* log_panic() force le vidage synchrone du tampon de log restant --
	 * sans ça, des messages encore en attente (mode différé) sont perdus
	 * au reset matériel de sys_poweroff(), comme observé le 2026-08-24
	 * ("N messages dropped" en sortie série après une rafale de logs). */
	log_panic();
	sys_poweroff();
