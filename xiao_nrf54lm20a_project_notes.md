...
*chute/choc, double-tap, bouton et yaw ne sont **pas câblés** — le driver
Zephyr `lsm6dsl` n'expose pas ces événements matériels via l'API
`sensor_trigger` standard ; le yaw nécessiterait une intégration
gyroscopique dans la durée. Envoyés à 0.
...
`0x16` (battery charging) n'est pas utilisé par ce firmware (jamais câblé).
