...
1. **Manipulation matérielle** : avant toute connexion du PPK II à une
   ...
   toucher au matériel.
   ⚠️ **Danger identifié (retour DevZone Nordic, carte nRF54L15-DK
   endommagée)** : le PPK II a deux modes — « source » (il fournit
   lui-même la tension) et « ampèremètre » (il mesure en série sur une
   alimentation déjà présente). Utiliser le PPK II en mode **source** à
   une tension dépassant le max admissible du point d'injection (ex.
   3,7V sur une broche VDD SoC dont le max est 3,6V) a détruit une carte
   dans le cas documenté. **Notre carte a déjà sa propre alimentation
   batterie** — utiliser le mode **ampèremètre**, jamais le mode source,
   sauf besoin explicitement justifié et tension vérifiée compatible
   avec le point d'injection exact.
...
et à jour du projet (build/flash/vérification, architecture System OFF,
registres IMU, trames BTHome, **§ « Budget énergétique (calculé, non
mesuré) »**). **À lire en premier** — c'est ce budget calculé que ce
document de transition vise à valider ou corriger par la mesure.
...
## Chiffres théoriques à valider (repris de `xiao_nrf54lm20a_project_notes.md`)
...
## Retours communautaires trouvés (recherche du 2026-08-30)

Recherche internet menée pour situer les attentes avant la session —
détail complet dans l'historique de conversation si besoin. Résumé :

- **Aucune mesure indépendante du System OFF pur du SoC n'existe encore
  pour cette carte précise** (trop récente) — notre mesure serait une
  première.
- Seule mesure PPK II tierce trouvée sur cette carte : forum Seeed,
  utilisateur *msfujino* (1er juillet 2026), PMIC nPM1300 en Hibernate
  (0,5 µA) et Ship mode (0,3 µA) — **pas le System OFF du SoC**, injection
  au connecteur BAT à 3,8V (mode ampèremètre implicite, sans incident).
- **Les chiffres System OFF publiés par Seeed elle-même sont
  incohérents entre leurs propres pages** (3,74 µA sur une page,
  4,76-4,93 µA sur une autre/un article qui les cite) — à ne pas prendre
  pour acquis, confirme l'intérêt d'une mesure indépendante.
- **Piège à vérifier en premier si une mesure paraît anormalement
  élevée** (retours DevZone Nordic sur nRF54L15, puce proche) : dans
  plusieurs cas documentés, un courant "trop haut" en sleep/System OFF
  venait de l'UART/logging resté actif (via `zephyr,console`/logging),
  pas d'un vrai défaut matériel — vérifier que le firmware suspend bien
  la console avant `sys_poweroff()` (notre firmware ne le fait
  actuellement pas explicitement, voir commentaire dans
  `xiao_door_sensor/src/main.c` sur ce point).
- Retour distinct (DevZone) sur un Xiao nRF54L15 : consommation qui
  augmente fortement si la tension d'alimentation appliquée est
  inférieure à ~3,3V (conception du convertisseur DC-DC de cette gamme
  de cartes) — vérifier la tension réellement appliquée pendant la
  mesure si un résultat semble trop élevé.
...
- Comparer aux chiffres théoriques ci-dessus, mettre à jour
  ...
  § « Prochaines étapes ») avec les valeurs mesurées. **Ne pas dupliquer
  le contenu de ce document dans cette conversation — le mettre à jour
  directement, et n'y garder que l'état actuel (valeurs mesurées), pas
  le récit de la session de mesure.**
